...
- **相似合并**（独立开关）：使用 NipaPlay 原生相似度引擎（C++ 实现，四级检测：完全相同 → 编辑距离 → 拼音距离 → 余弦相似度），在时间窗口内匹配相似弹幕，合并为单条并标注合并数，替代原生合并渲染。合并时选择频次最高的文本作为代表，避免偏门弹幕代表主流内容。
...
- **拼音对比**：原生引擎内置 6763 条拼音字典，自动识别中文同音变体（如"哈哈哈"与"哈哈哈"），JS 端无法实现的拼音匹配现已开箱即用。
...
- **异常回退**：原生引擎调用包裹在 `try/catch` 中，不可用时回退到不合并。
